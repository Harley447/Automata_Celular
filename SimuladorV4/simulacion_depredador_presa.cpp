...
void guardarGraficaPNG(const std::vector<int>& gen, const std::vector<int>& presas,
    ...
    if (fuenteCargada) {
        ...
        // Etiqueta Eje X (Tiempo)
        sf::Text etiquetaX(fuente, "Tiempo (Generaciones)", 16);
        ...
        // Etiqueta Eje Y (Poblacion)
        sf::Text etiquetaY(fuente, "Poblacion", 16);
        etiquetaY.setFillColor(sf::Color::Black);
        etiquetaY.setRotation(sf::degrees(-90.f)); 
        etiquetaY.setPosition(sf::Vector2f(15, alto / 2.0f + etiquetaY.getLocalBounds().size.x / 2.0f));
